...
//#define NGSS_USE_RECEIVER_PLANE_BIAS//If enabled you better disable light Bias and Normal Bias entirelly. Otherwise you are biasing shadows twice
#if defined(NGSS_USE_RECEIVER_PLANE_BIAS)
#define NGSS_RECEIVER_PLANE_BIAS 0.1
#endif

uniform float NGSS_CASCADE_BLEND_DISTANCE = 0.25;

#define NGSS_USE_POISSON_SAMPLING_DIR
uniform float NGSS_POISSON_SAMPLING_NOISE_DIR = 1.0;

//We dont need to bail out lower than 32 samplers
#if defined(NGSS_USE_EARLY_BAILOUT_OPTIMIZATION_DIR) && (defined(DIR_POISSON_32) || defined(DIR_POISSON_64))
#define NGSS_CAN_USE_EARLY_BAILOUT_OPTIMIZATION_DIR
#endif

#if UNITY_VERSION > 565 && defined(NGSS_PCSS_FILTER_DIR) && (SHADER_TARGET >= 35) && !defined(SHADER_API_GLCORE)
SamplerState my_point_clamp_smp2;
#define NGSS_CAN_USE_PCSS_FILTER_DIR
#endif

uniform float NGSS_PCSS_GLOBAL_SOFTNESS = 0.01;
...
{
	#if defined(NGSS_NOISE_STATIC_DIR)
	return DirRandValue01(seed);
	#else
	return DirRandValue01(seed) * NGSS_POISSON_SAMPLING_NOISE_DIR * 0.05;// clamp(NGSS_POISSON_SAMPLING_NOISE_DIR, 0.0, 10.0);
	#endif
}

float3 UnityGetReceiverPlaneDepthBiasNGSS(float3 shadowCoord, float biasMultiply)
{
	// Should receiver plane bias be used? This estimates receiver slope using derivatives,
	...
	biasUVZ.z = -min(fractionalSamplingError, NGSS_RECEIVER_PLANE_BIAS);
#if defined(UNITY_REVERSED_Z)
	biasUVZ.z *= -1;
#endif
#endif
...
#if defined(NGSS_USE_POISSON_SAMPLING_DIR)		
...
#if defined(NGSS_CAN_USE_EARLY_BAILOUT_OPTIMIZATION_DIR) && !defined(NGSS_CAN_USE_PCSS_FILTER_DIR)//We dont need to overoptimize with too many branches
	UNITY_LOOP
	for (int i = 0; i < 16; i++)
	{
...
#endif//NGSS_CAN_USE_EARLY_BAILOUT_OPTIMIZATION_DIR
...
//Main Function
float NGSS_Main(float4 coords, float3 receiverPlaneDepthBias, float c, float s)
{
	float shadowSoftness = NGSS_PCSS_GLOBAL_SOFTNESS + 0.001;
	float2 uv = coords.xy;
	float receiver = coords.z;

	float Sampler_Number = Dir_Samplers_Count;//(int)clamp(Dir_Samplers_Count * (shadowSoftness / NGSS_PCSS_GLOBAL_SOFTNESS), Dir_Samplers_Count * 0.5, Dir_Samplers_Count);
	...
	if (blockerResults.y < 1.0)//There are no occluders so early out (this saves filtering)
		return 1.0;
#if defined(NGSS_CAN_USE_EARLY_BAILOUT_OPTIMIZATION_DIR)
	...
	float diskRadius = clamp(penumbra, NGSS_PCSS_FILTER_DIR_MIN, NGSS_PCSS_FILTER_DIR_MAX) * shadowSoftness;
#else
	float diskRadius = shadowSoftness * 0.15;//NO PCSS FILTERING
#endif//NGSS_CAN_USE_PCSS_FILTER_DIR

	//Sampler_Number = (int)clamp(Sampler_Number * (diskRadius / NGSS_PCSS_GLOBAL_SOFTNESS), Sampler_Number * 0.5, Sampler_Number);
...
*/
	...
	//float4 rotation = tex2D(unity_RandomRotation16, i.uv.xy * _ScreenParams.xy * (NGSS_POISSON_SAMPLING_NOISE_DIR + 0.001));
	float4 rotation = tex2D(unity_RandomRotation16, i.uv.xy * NGSS_POISSON_SAMPLING_NOISE_DIR);
...
#endif
*/
	float shadow = NGSS_Main(coord, receiverPlaneDepthBias, c, s);
	...
	*/
	...
	if (alpha > 1.0 - NGSS_CASCADE_BLEND_DISTANCE)
	{
		...
		receiverPlaneDepthBias = UnityGetReceiverPlaneDepthBiasNGSS(coordCascade0.xyz, biasMultiply);
#endif

		//half shadowNextCascade = UnitySampleShadowmap_PCF3x3(coord, receiverPlaneDepthBias);
		half shadowNextCascade = NGSS_Main(coord, receiverPlaneDepthBias, c, s);
...
Subshader
{
	...
	Pass
	{
		...
		#pragma shader_feature NGSS_NOISE_STATIC_DIR
