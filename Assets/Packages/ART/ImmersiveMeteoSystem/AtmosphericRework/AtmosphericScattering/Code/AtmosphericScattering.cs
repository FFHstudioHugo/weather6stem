...
public class AtmosphericScattering : MonoBehaviour {
    ...
    public ImmersiveWeatherControl weather;
    float rayleightdensity;
    float rayleighcolor;
    float miePower; 
public Light sun;
    ...
    private void Update()
    {
        rayleightdensity = weather.RayleighDensity*weather.AtmosphericPower;
        rayleighcolor = weather.RayleighColor * weather.AtmosphericPower;
        miePower = weather.MiePower * weather.AtmosphericPower;
        ...
        worldScaleExponent = Mathf.Clamp(worldScaleExponent, 0f, 1.1f);
        ...
        worldRayleighDensity = Mathf.Clamp(rayleightdensity, 0, 1000f);
	...
	//	EnsureHookedLightSource(activeSun.light);
		...
		sun.AddCommandBuffer(UnityEngine.Rendering.LightEvent.AfterShadowMap, m_occlusionCmdAfterShadows);
	sun.AddCommandBuffer(UnityEngine.Rendering.LightEvent.BeforeScreenspaceMask, m_occlusionCmdBeforeScreen);
	}
   ...
   light.RemoveCommandBuffer(UnityEngine.Rendering.LightEvent.AfterShadowMap, m_occlusionCmdAfterShadows);
     light.RemoveCommandBuffer(UnityEngine.Rendering.LightEvent.BeforeScreenspaceMask, m_occlusionCmdBeforeScreen);
 /*
        if (m_occlusionCmdAfterShadows != null)
			m_occlusionCmdAfterShadows.Dispose();
		if(m_occlusionCmdBeforeScreen != null)
	...
	void OnDisable() {
		...
		//var activeSun = AtmosphericScatteringSun.instance;
	;
		sun.RemoveCommandBuffer(UnityEngine.Rendering.LightEvent.AfterShadowMap, m_occlusionCmdAfterShadows);
		sun.RemoveCommandBuffer(UnityEngine.Rendering.LightEvent.BeforeScreenspaceMask, m_occlusionCmdBeforeScreen);
	}
	...
	void UpdateStaticUniforms() {
		...
		var rayleighColorP10 = worldRayleighColorRamp.Evaluate(0.70f);
		var rayleighColorP20 = worldRayleighColorRamp.Evaluate(0.90f);
		
		var mieColorM20 = worldMieColorRamp.Evaluate(0.00f);
		var mieColorO00 = worldMieColorRamp.Evaluate(0.40f);
		var mieColorP20 = worldMieColorRamp.Evaluate(0.90f);
		
		Shader.SetGlobalVector("u_RayleighColorM20", (Vector4)rayleighColorM20 * rayleighcolor);
		Shader.SetGlobalVector("u_RayleighColorM10", (Vector4)rayleighColorM10 * rayleighcolor);
		Shader.SetGlobalVector("u_RayleighColorO00", (Vector4)rayleighColorO00 * rayleighcolor);
		Shader.SetGlobalVector("u_RayleighColorP10", (Vector4)rayleighColorP10 * rayleighcolor);
		Shader.SetGlobalVector("u_RayleighColorP20", (Vector4)rayleighColorP20 * rayleighcolor);
		
		Shader.SetGlobalVector("u_MieColorM20", (Vector4)mieColorM20 * miePower);
		Shader.SetGlobalVector("u_MieColorO00", (Vector4)mieColorO00 * miePower);
		Shader.SetGlobalVector("u_MieColorP20", (Vector4)mieColorP20 * miePower);
