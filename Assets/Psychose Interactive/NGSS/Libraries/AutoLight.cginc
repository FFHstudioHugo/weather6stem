...
// ---- Screen space direction light shadows helpers (any version)
        ...
        inline fixed unitySampleShadowNGSS (unityShadowCoord4 shadowCoord, float2 screenPos)
        {
			...
			float diskRadius = NGSS_GLOBAL_SOFTNESS_OPTIMIZED * 0.00065f;//(1.0 - _LightShadowData.r);
			...
			//diskRadius *= 0.0175;
			return PCF_FILTER_SPOT_OPTIMIZED(coord, diskRadius, randPied, NGSS_OPTIMIZED_SAMPLERS) + _LightShadowData.r;
		#else
			/*************************************************************************************/
			...
			//declare stuff
			float mSize = NGSS_OPTIMIZED_ITERATIONS;//incoming number is always odd
			...
			float softness = NGSS_GLOBAL_SOFTNESS_OPTIMIZED / _ShadowMapTexture_TexelSize.zw;
        ...
        {
			float2 spos = tex2D(unity_RandomRotation16, shadowCoord.xy * _ScreenParams.xy * 16).xy;// shadowCoord.xy;
